  ...
  return (
    <form className="storeForm productForm" onSubmit={submit}>
      ...
      <fieldset className="imageUploadFields">
        ...
        <p>Ajoutez jusqu’à 3 photos JPG, PNG ou WebP. La première sera l’image principale.</p>
        ...
        {photos.length > 0 && (
          <div className="photoPreviewGrid">
            {photos.map((photo, index) => (
              <article className="photoPreviewCard" key={photo.url}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={photo.url} alt={`Photo ${index + 1} du produit`} />
                {index === 0 && <span>Image principale</span>}
                <button type="button" onClick={() => removePhoto(index)} aria-label={`Supprimer la photo ${index + 1}`}>×</button>
              </article>
            ))}
          </div>
        )}
