...
export default function VerifyEmailPage() {
  ...
  return <main className="authPage"><section className="authBrand"><a className="authLogo" href={localizedHome(locale)}>Todijo<span>.</span></a><div className="authPitch"><h1>{t("verificationTitle")}</h1><p>{t(resultKey)}</p></div><small>© 2026 Todijo</small></section><section className="authPanel"><div className="authBox"><a className="authBack" href={localizedHome(locale)}>← {t("back")}</a><h2>{t("verificationTitle")}</h2><p className="authMessage isSuccess" role="status">{t(resultKey)}</p>{status !== "success" && <><h3>{t("resendTitle")}</h3><p className="authIntro">{t("resendIntro")}</p>{sent ? <p className="authMessage isSuccess" role="status">{t("resendNeutral")}</p> : <form className="authForm" onSubmit={resend}><div className="formField"><label htmlFor="email">{t("email")}</label><input id="email" name="email" type="email" autoComplete="email" required/></div><button className="authSubmit" type="submit" disabled={sending}>{sending ? t("resendSending") : t("resendSubmit")}</button></form>}</>}</div></section></main>;
}
