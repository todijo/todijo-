...
export default async function MessagesPage() {
  ...
  const content=<section className="messagesShell"><div className="messagesHeading"><DashboardReturnLink href={`/${locale}/dashboard`} label={session.role === "CUSTOMER" ? dashboard("buyerArea") : dashboard("sellerDashboard")}/><p className="dashboardBadge">{common("messages")}</p><h1>{dashboard("myConversations")}</h1></div>
