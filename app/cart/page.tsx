...
export default function CartPage() {
  ...
  const sellerGroups = useMemo(() => {
    ...
    return [...groups.entries()].map(([key, lines]) => ({ key, lines, subtotal: lines.reduce((sum, line) => sum + (line.requiresAuthoritativePrice && !line.authoritativePrice ? 0 : line.price * line.quantity), 0) }));
  }, [items]);
