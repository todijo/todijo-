...
export default async function CheckoutSuccessPage({ searchParams }: { searchParams: Promise<{ session_id?: string }> }) {
  ...
  const order = auth && sessionId ? await prisma.order.findFirst({
    ...
    select: {
      ...
      items: { select: { id: true, quantity: true, unitPrice: true, productNameSnapshot: true, productImageUrlSnapshot: true, selectedColor: true, selectedSize: true, selectedOptions: true, product: { select: { name: true, images: true, store: { select: { name: true } } } } } },
    },
  }) : null;
