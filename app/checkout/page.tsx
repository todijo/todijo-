...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/shipping/quote", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ items:items.map(item=>({productId:item.id,quantity:item.quantity})), destinationCountry,destinationPostalCode }) })
      ...
      .then(({ok,data}) => { if (!active) return; if (!ok) { setQuote(null); setError(data.code === "SHIPPING_POSTAL_UNAVAILABLE" ? shipping("postalUnavailable") : data.code === "SHIPPING_DESTINATION_UNAVAILABLE" ? shipping("destinationUnavailable") : data.code === "MULTIPLE_SELLERS" ? connect("multipleSellers") : shipping("notConfigured")); } else setQuote(data as NonNullable<typeof quote>); })
  ...
  return <main className="checkoutPage"><SiteHeader /><section className="checkoutShell">
    ...
    {items.length === 0 ? <div className="emptyCartCard"><div>🛒</div><h2>{t("empty")}</h2><Link className="primary" href="/">{t("discover")}</Link></div> : <div className="checkoutGrid">
      <section className="checkoutForm">
        <section className="checkoutShipping"><div className="checkoutStep"><span>1</span><div><h2>{shipping("destination")}</h2><p>{shipping("destinationHelp")}</p></div></div><div className="sellerControlFieldGrid"><label>{shipping("countryCode")}<input value={destinationCountry} onChange={(event)=>setDestinationCountry(event.target.value.toUpperCase().replace(/[^A-Z]/g,"").slice(0,2))} maxLength={2} placeholder="FR" inputMode="text" autoComplete="country"/></label><label>{shipping("postalCode")}<input value={destinationPostalCode} onChange={event=>setDestinationPostalCode(event.target.value)} maxLength={16} autoComplete="postal-code"/></label></div>{quoteLoading&&<p>{shipping("checking")}</p>}{quote&&<div className="shippingQuoteCard"><strong>{quote.method}</strong>{quote.carrier&&<span>{quote.carrier}</span>}<span>{shipping("estimate",{min:quote.estimatedMinDays,max:quote.estimatedMaxDays})}</span><b>{quote.free?shipping("freeLabel"):formatCurrency(Number(quote.amount),quote.currency,locale)}</b></div>}</section>
