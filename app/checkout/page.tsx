...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    if (!items.length || !/^[A-Z]{2}$/.test(destinationCountry)) { setQuote(null); return; }
    let active = true; setQuoteLoading(true); setError("");
    fetch("/api/shipping/quote", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ productIds: items.map((item)=>item.id), destinationCountry }) })
      ...
      .then(({ok,data}) => { if (!active) return; if (!ok) { setQuote(null); setError(data.code === "SHIPPING_DESTINATION_UNAVAILABLE" ? shipping("destinationUnavailable") : data.code === "MULTIPLE_SELLERS" ? connect("multipleSellers") : shipping("notConfigured")); } else setQuote(data as NonNullable<typeof quote>); })
      .catch(()=>{ if (active) { setQuote(null); setError(shipping("quoteError")); } }).finally(()=>{ if (active) setQuoteLoading(false); });
    return () => { active = false; };
  }, [destinationCountry, items, shipping, connect]);
  ...
  async function beginCheckout() {
    ...
    try {
      const response = await fetch("/api/checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ requestId, destinationCountry, items: items.map((item) => ({ productId: item.id, quantity: item.quantity, selectedColor: item.selectedColor, selectedSize: item.selectedSize, variantId: item.variantId })) }) });
      const result = await response.json() as { url?: string; error?: string; code?: string };
      if (!response.ok || !result.url) throw new Error(result.code === "MULTIPLE_SELLERS" ? connect("multipleSellers") : result.code === "SELLER_STRIPE_NOT_READY" ? connect("sellerNotReady") : result.code === "SELLER_STATUS_REQUIRED" ? sellerTransparency("checkoutBlocked") : result.code === "SHIPPING_DESTINATION_UNAVAILABLE" ? shipping("destinationUnavailable") : result.code === "SHIPPING_NOT_CONFIGURED" ? shipping("notConfigured") : t("startError"));
  ...
  return <main className="checkoutPage"><SiteHeader /><section className="checkoutShell">
    ...
    {items.length === 0 ? <div className="emptyCartCard"><div>🛒</div><h2>{t("empty")}</h2><Link className="primary" href="/">{t("discover")}</Link></div> : <div className="checkoutGrid">
      <section className="checkoutForm">
        <section className="checkoutShipping"><div className="checkoutStep"><span>1</span><div><h2>{shipping("destination")}</h2><p>{shipping("destinationHelp")}</p></div></div><label>{shipping("countryCode")}<input value={destinationCountry} onChange={(event)=>setDestinationCountry(event.target.value.toUpperCase().replace(/[^A-Z]/g,"").slice(0,2))} maxLength={2} placeholder="FR" inputMode="text" autoComplete="country"/></label>{quoteLoading&&<p>{shipping("checking")}</p>}{quote&&<div className="shippingQuoteCard"><strong>{quote.method}</strong>{quote.carrier&&<span>{quote.carrier}</span>}<span>{shipping("estimate",{min:quote.estimatedMinDays,max:quote.estimatedMaxDays})}</span><b>{quote.free?shipping("freeLabel"):formatCurrency(Number(quote.amount),quote.currency,locale)}</b></div>}</section>
