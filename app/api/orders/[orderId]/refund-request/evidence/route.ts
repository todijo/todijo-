...
import { listBuyerRefundEvidence, RefundEvidenceError, uploadBuyerRefundEvidence } from "@/lib/refund-evidence";
...
function errorResponse(error: unknown, fallback: string) {
  return NextResponse.json({ error: error instanceof RefundEvidenceError ? error.message : fallback }, { status: error instanceof RefundEvidenceError ? error.status : 500 });
}
