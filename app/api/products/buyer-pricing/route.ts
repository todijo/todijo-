...
export async function POST(request:Request){
 ...
 const prices=[] as Array<{productId:string;variantId:string|null;kind:string;amount:string;currency:string}>;
 for(const item of items){const product=products.find(candidate=>candidate.id===item.productId);if(!product||product.supplierLink?.provider==="CJ"&&product.supplierLink.ownerType==="PLATFORM")continue;const variant=item.variantId?product.variants.find(candidate=>candidate.id===item.variantId&&candidate.active):null;if(item.variantId&&!variant)continue;const rule=effectiveShippingRule(product.store,product),source=item.kind==="shippingPrice"?rule.shippingPrice:item.kind==="freeThreshold"?rule.shippingFreeThreshold:variant?.priceOverride??product.price;if(source==null)continue;const presentment=await convertMarketplacePrice(source,item.kind==="productPrice"?product.currency:product.store.currency,market.currency);prices.push({productId:product.id,variantId:variant?.id??null,kind:item.kind,amount:presentment.buyerAmount,currency:presentment.buyerCurrency});}
