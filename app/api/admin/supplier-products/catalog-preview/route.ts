...
async function cjGet(path:string){
  for(let attempt=0;attempt<2;attempt+=1){
    ...
    const response=await scheduleCjRequest("read",()=>fetch(`${CJ_BASE_URL}${path}`,{headers:{"CJ-Access-Token":token,"Accept":"application/json"},signal:AbortSignal.timeout(PREVIEW_TIMEOUT_MS),cache:"no-store"}));
    ...
    if(authFailed&&attempt===0){cjAuth.invalidateAccessToken();continue;}
    ...
    if(!response.ok||payload.result===false||payload.success===false)throw new Error(response.status>=500?"CJ_UNAVAILABLE":"CJ_API_REQUEST_FAILED");
    return payload.data;
  }
...
async function resolvePreviewProduct(identifier:string){
  ...
  try{
    return await cjGet(`/product/query?${isSku?"productSku":"pid"}=${encodeURIComponent(identifier)}&features=enable_video`);
  }catch(error){
    ...
    if(pids.length===0)throw new Error("CJ_PRODUCT_NOT_FOUND");
    if(pids.length>1)throw new Error("CJ_PRODUCT_IDENTIFIER_AMBIGUOUS");
...
export async function POST(request:Request){
  try{
    ...
    for(const identifier of identifiers){
      try{
        const product=await resolvePreviewProduct(identifier);
        const snapshot=normalizeCjProduct(product,[],[]);
        if(!snapshot.supplierProductId)throw new Error("CJ_PRODUCT_NOT_FOUND");
