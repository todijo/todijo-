...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (partition.kind === "static") entities = [{ pathname: "" }, { pathname: "store" }, ...infoSlugs.map((slug) => ({ pathname: `info/${slug}` }))];
    ...
    if (partition.kind === "products") {
      const products = await prisma.product.findMany({ where: { status: "PUBLISHED", ...publicProductAccessWhere(now) }, orderBy: [{ createdAt: "asc" }, { id: "asc" }], skip, take: SITEMAP_ENTITY_CHUNK_SIZE, select: { id: true,name:true,description:true,sourceLocale:true,translations:{select:{locale:true,title:true,description:true,automatic:true}},updatedAt: true,supplierLink:{select:{sourceMetadata:true}} } });
      entities = products.map((product) => ({ pathname: `product/${product.id}/${productSlug(product.name)}`,localePathnames:Object.fromEntries(locales.map(locale=>[locale,`product/${product.id}/${productSlug(resolveBuyerProductContent({name:product.name,description:product.description,sourceMetadata:product.supplierLink?.sourceMetadata,locale,sourceLocale:product.sourceLocale,translations:product.translations}).title)}`])), lastModified: product.updatedAt }));
    } else {
