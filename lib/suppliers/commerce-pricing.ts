...
type Provider=Pick<CjCatalogProvider,"getVariant"|"calculateFreight">;
...
export function resolveCanonicalSupplierVariant(input:{supplierProductId:string|null;connectionId:string|null;variant:{supplierVariantId:string|null;supplierConnectionId:string|null}|null}){
 const variant=input.variant;
 if(!input.supplierProductId||!input.connectionId||!variant?.supplierVariantId||variant.supplierConnectionId!==input.connectionId)throw new DropshippingCommerceError("DROPSHIPPING_VARIANT_INVALID");
 return{supplierProductId:input.supplierProductId,supplierVariantId:variant.supplierVariantId};
}
...
export async function resolveDropshippingPricing(db:PrismaClient,input:{productId:string;variantId:string;quantity:number;destinationCountry:unknown;buyerCurrency:unknown},dependencies:{provider?:Provider;fx?:typeof verifiedFxRate;allowUnpublished?:boolean;targetMargin?:Prisma.Decimal.Value}={}):Promise<ResolvedDropshippingPricing>{
 ...
 const variant=product.variants[0];if(!variant)throw new DropshippingCommerceError("DROPSHIPPING_VARIANT_INVALID");
 const canonical=resolveCanonicalSupplierVariant({supplierProductId:link!.supplierProductId,connectionId:link!.connectionId,variant});
 const provider=dependencies.provider??new CjCatalogProvider();
 try{
  const supplierVariant=await provider.getVariant(canonical.supplierVariantId);
  if(supplierVariant.supplierProductId!==canonical.supplierProductId||supplierVariant.supplierVariantId!==canonical.supplierVariantId)throw new DropshippingCommerceError("DROPSHIPPING_VARIANT_INVALID");
  if(supplierVariant.cost==null||!supplierVariant.available)throw new DropshippingCommerceError("DROPSHIPPING_COST_UNAVAILABLE");
