...
import { classifyCjProduct } from "./cj-classification";
...
export async function processCatalogImportJob(db:PrismaClient,provider:SupplierCatalogProvider,jobId:string,input:{adminId:string;limit?:unknown}){
  ...
  for(let processed=0;processed<limit;processed+=1){
    ...
    try{
      const snapshot=await provider.getProduct(candidate.requestedIdentifier),classification=classifyCjProduct(snapshot),automaticCategory=classification.status==="SUGGESTED"?classification.canonicalCategoryId:null,category=resolveCatalogCategory(snapshot,candidate.canonicalCategoryId??automaticCategory),compliance=catalogComplianceDecision(snapshot);
      const common={canonicalSupplierId:snapshot.supplierProductId,supplierSku:snapshot.sku,canonicalCategoryId:category.categoryId,categoryMappingSource:candidate.canonicalCategoryId?"ADMIN":automaticCategory?"CLASSIFIER":category.source,categoryMappingReason:candidate.canonicalCategoryId?"ADMIN_SELECTED_CANONICAL_LEAF":automaticCategory?"CLASSIFIER_ACCEPTED_THRESHOLD":category.reason,classificationStatus:candidate.canonicalCategoryId?"ADMIN_REVIEWED":classification.status,classificationConfidence:classification.confidence,classificationEvidence:classification.evidence as Prisma.InputJsonValue,stockStatus:snapshot.available&&snapshot.stock>0?"AVAILABLE":"UNAVAILABLE",complianceStatus:compliance.status,complianceReason:compliance.reason};
      ...
      const product=await importSupplierProduct(db,provider,media,{storeId:job.storeId,connectionId:PLATFORM_CJ_CONNECTION_ID,ownerType:"PLATFORM",supplierProductId:snapshot.supplierProductId,sellingPrice:null,sellingCurrency:job.store.currency,category:category.categoryId,snapshot,classification});
