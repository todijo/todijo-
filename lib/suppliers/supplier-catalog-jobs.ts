...
export async function processCatalogImportJob(db:PrismaClient,provider:SupplierCatalogProvider,jobId:string,input:{adminId:string;limit?:unknown},dependencies:{media?:ReturnType<typeof defaultSupplierMediaProvider>;importer?:typeof importSupplierProduct}={}){
  ...
  for(let processed=0;processed<limit;processed+=1){
    ...
    try{
      ...
      if(existing){await db.supplierCatalogImportItem.update({where:{id:candidate.id},data:{...common,status:"SKIPPED",pricingStatus:"NOT_REQUIRED_EXISTING_PRODUCT",productId:existing.productId,errorCode:"SUPPLIER_PRODUCT_ALREADY_IMPORTED",completedAt:new Date()}});continue;}
      ...
      let pricing:Awaited<ReturnType<typeof verifiedCatalogPricing>>;try{pricing=await verifiedCatalogPricing(provider,snapshot,job.destinationCountry,job.store.currency);}catch(error){const code=publicCatalogError(error),evidence=error instanceof CatalogPricingResolutionError?error.evidence:null;console.warn("[cj-catalog-pricing]",JSON.stringify({event:"catalog_pricing_unavailable",jobId,itemId:candidate.id,supplierProductId:snapshot.supplierProductId,variantsExamined:evidence?.variantsExamined??snapshot.variants.length,eligibleVariants:evidence?.eligibleVariants??null,terminalErrorCode:code}));await db.supplierCatalogImportItem.update({where:{id:candidate.id},data:{...common,status:"QUARANTINED",pricingStatus:"UNAVAILABLE",pricingEvidence:evidence?evidence as Prisma.InputJsonValue:Prisma.DbNull,errorCode:code,errorMessage:code,completedAt:new Date()}});continue;}
      const product=await importer(db,provider,media,{storeId:job.storeId,connectionId:PLATFORM_CJ_CONNECTION_ID,ownerType:"PLATFORM",supplierProductId:snapshot.supplierProductId,sellingPrice:null,sellingCurrency:job.store.currency,category:category.categoryId,snapshot,classification});
