...
export function catalogJobProgress(job:CatalogJobProgressInput,now=Date.now()){
  ...
  const startedAt=timestamp(job.startedAt),terminal=job.status==="COMPLETED"||job.status==="COMPLETED_WITH_ERRORS"||job.status==="CANCELLED",endedAt=terminal?timestamp(job.completedAt):job.isProcessing?now:timestamp(job.updatedAt);
