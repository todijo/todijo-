...
for(const leaf of MARKETPLACE_CANONICAL_LEAF_CATEGORIES){const key=normalized(leaf.label),current=leafByExactLabel.get(key);leafByExactLabel.set(key,current===undefined?leaf.id:null);}
...
export function resolveCatalogCategory(snapshot:Pick<SupplierProductSnapshot,"categoryReference"|"title">,adminCategoryId?:string|null):CatalogCategoryDecision{
  ...
  if(explicit){if(!isMarketplaceCanonicalLeafCategoryId(explicit))return{categoryId:null,source:"UNMAPPED",reason:"CANONICAL_CATEGORY_INVALID"};return{categoryId:explicit,source:"ADMIN",reason:"ADMIN_SELECTED_CANONICAL_LEAF"};}
  const source=snapshot.categoryReference?.trim()??"";
  if(source&&isMarketplaceCanonicalLeafCategoryId(source))return{categoryId:source,source:"EXACT_SOURCE",reason:"SOURCE_MATCHED_CANONICAL_ID"};
...
const prohibitedPatterns=[/\b(gun|rifle|pistol|ammunition|weapon|switchblade)\b/i,/\b(adult toy|sex toy|pornographic)\b/i,/\b(prescription|medical cure|cures cancer|weight loss drug)\b/i,/\b(counterfeit|replica logo|fake brand)\b/i];
...
export function canonicalCategorySummary(categoryId:string|null){const leaf=categoryId?marketplaceCanonicalLeafCategory(categoryId):null;return leaf?{id:leaf.id,main:leaf.categoryLabel,group:leaf.groupLabel,leaf:leaf.label}:null;}
export function publicCatalogError(error:unknown){const code=error instanceof Error?error.message:"SUPPLIER_IMPORT_FAILED";const allowed=new Set(["SUPPLIER_PRODUCT_ALREADY_IMPORTED","CJ_PRODUCT_NOT_FOUND","CJ_PRODUCT_IDENTIFIER_AMBIGUOUS","CJ_PRODUCT_ID_INVALID","SUPPLIER_PRODUCT_INVALID","SUPPLIER_NOT_CONFIGURED","PRICING_COST_INVALID","PRICING_CURRENCY_CONVERSION_REQUIRED","FX_NOT_CONFIGURED","FX_UNAVAILABLE","FX_RATE_STALE","FX_RATE_MISSING","CJ_FREIGHT_UNAVAILABLE","CJ_FREIGHT_NO_METHODS","CJ_FREIGHT_RESPONSE_INVALID","CJ_FREIGHT_TEMPORARY_FAILURE","CJ_FREIGHT_ALL_ORIGINS_UNAVAILABLE","CJ_CATALOG_PRICING_UNAVAILABLE","DROPSHIPPING_NO_PURCHASABLE_VARIANT","DROPSHIPPING_ORIGIN_OR_COST_UNAVAILABLE","CJ_UNAVAILABLE","CJ_API_REQUEST_FAILED"]);return allowed.has(code)?code:"SUPPLIER_IMPORT_FAILED";}
