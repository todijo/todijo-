...
export default function ProductCardAction({ product, className = "" }: { product: ProductCardActionProduct; className?: string }) {
  ...
  if (action === "CHOOSE_OPTIONS"||product.requiresAuthoritativePrice) {
    return <a className={`${classes} cardChooseOptionsButton`} href={productCardOptionHref(product.id, locale)} aria-label={`${t("chooseOptions")}: ${product.name}`}><ShoppingCart size={18} aria-hidden="true"/><span className="cardActionLabel">{t("chooseOptions")}</span></a>;
  }

  if (action === "SOLD_OUT" || product.stock == null) {
    return <button type="button" className={classes} disabled aria-label={`${t("unavailable")}: ${product.name}`}><ShoppingCart size={18} aria-hidden="true"/><span className="cardActionLabel">{t("unavailable")}</span></button>;
  }

  const stock = product.stock;
  return <button type="button" className={classes} onClick={() => addItem({ ...product, stock })} aria-label={`${t("add")}: ${product.name}`}><ShoppingCart size={18} aria-hidden="true"/><span className="cardActionLabel">{t("add")}</span></button>;
}
