...
export function SellerFulfillmentControl({ orderId, action }: { orderId: string; action: "PAID" | "PROCESSING" | "SHIPPED" }) {
  ...
  async function submit() {
    ...
    try {
      const payload = action === "PROCESSING" ? { action, trackingCarrier: carrier, trackingNumber: tracking, trackingUrl } : { action };
      ...
      if (action === "PROCESSING") { setCarrier(""); setTracking(""); setTrackingUrl(""); }
      router.refresh();
    } catch {
  ...
  return <div className="sellerFulfillmentControl">{action === "PROCESSING" && <><input value={carrier} onChange={(event) => setCarrier(event.target.value)} maxLength={120} placeholder={t("fulfillment.trackingCarrier")} aria-label={t("fulfillment.trackingCarrier")}/><input value={tracking} onChange={(event) => setTracking(event.target.value)} maxLength={160} placeholder={t("fulfillment.trackingNumber")} aria-label={t("fulfillment.trackingNumber")}/><input value={trackingUrl} onChange={(event) => setTrackingUrl(event.target.value)} maxLength={500} type="url" placeholder="https://…" aria-label={shipping("trackingUrl")}/></>}<button className="premiumTextLink" type="button" onClick={submit} disabled={saving}>{saving ? t("fulfillment.updating") : label}</button>{error && <p role="alert">{error}</p>}</div>;
}
