...
export function SellerFulfillmentControl({ orderId, action }: { orderId: string; action: "PAID" | "PROCESSING" | "SHIPPED" }) {
  ...
  return <div className="sellerFulfillmentControl">{action === "PROCESSING" && <><input value={carrier} onChange={(event) => setCarrier(event.target.value)} placeholder={t("fulfillment.trackingCarrier")} aria-label={t("fulfillment.trackingCarrier")}/><input value={tracking} onChange={(event) => setTracking(event.target.value)} placeholder={t("fulfillment.trackingNumber")} aria-label={t("fulfillment.trackingNumber")}/></>}<button className="premiumTextLink" type="button" onClick={submit} disabled={saving}>{saving ? t("fulfillment.updating") : label}</button>{error && <p role="alert">{error}</p>}</div>;
}
