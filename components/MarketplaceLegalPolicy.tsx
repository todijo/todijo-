import { AlertTriangle, FileText, MessageCircle, ShieldCheck } from "lucide-react";

type LegalSection = { title: string; body: string };

export default function MarketplaceLegalPolicy({ title, intro, statusNote, traderNote, protection, sections, contactHref, contactLabel, relatedLinks=[] }: { title: string; intro: string; statusNote: string; traderNote?: string; protection?: {title:string;body:string}; sections: LegalSection[]; contactHref: string; contactLabel: string; relatedLinks?:Array<{label:string;href:string}> }) {
  return <article className="marketInfoContent legalPolicy">
    ...
    {protection && <section className="legalProtectionSummary" aria-labelledby="buyer-protection-summary"><ShieldCheck size={24} aria-hidden="true"/><div><h2 id="buyer-protection-summary">{protection.title}</h2><p>{protection.body}</p></div></section>}
