...
export default function SellerCategorySelector({initialValue="",labels}:{initialValue?:string;labels:Labels}){
  ...
  return <div className="sellerCategorySelector">
    <div className="sellerCategoryLevel"><label htmlFor="category-main">{labels.main}</label><select id="category-main" value={categoryId} required onChange={event=>{setCategoryId(event.target.value);setGroupId("");setLeafId("")}}><option value="" disabled>{labels.chooseMain}</option>{DESKTOP_CATEGORY_TAXONOMY.map(item=><option key={item.id} value={item.id}>{item.label}</option>)}</select></div>
    <div className="sellerCategoryLevel"><label htmlFor="category-group">{labels.group}</label><select id="category-group" value={groupId} required disabled={!category} onChange={event=>{setGroupId(event.target.value);setLeafId("")}}><option value="" disabled>{labels.chooseGroup}</option>{category?.groups.map(item=><option key={item.id} value={item.id}>{item.label}</option>)}</select></div>
    <div className="sellerCategoryLevel"><label htmlFor="category">{labels.leaf}</label><select id="category" name="category" value={leafId} required disabled={!group} onChange={event=>setLeafId(event.target.value)}><option value="" disabled>{labels.chooseLeaf}</option>{group?.items.map(label=>{const id=subcategoryId(categoryId,groupId,label);return <option key={id} value={id}>{label}</option>})}</select></div>
