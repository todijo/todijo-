...
type JobSummary={id:string;status:string;requestedCount:number;processedCount:number;importedCount:number;skippedCount:number;quarantinedCount:number;failedCount:number;batchLimit:number;destinationCountry:string;createdAt:string|Date;startedAt?:string|Date|null;updatedAt:string|Date;completedAt?:string|Date|null};
...
function elapsedLabel(job:JobSummary){const start=new Date(job.startedAt??job.createdAt).getTime(),end=job.completedAt?new Date(job.completedAt).getTime():new Date(job.updatedAt).getTime(),seconds=Math.max(0,Math.round((end-start)/1000));return seconds<60?`${seconds}s`:`${Math.floor(seconds/60)}m ${seconds%60}s`;}
