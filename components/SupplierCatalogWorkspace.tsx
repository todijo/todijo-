  ...
  return <section className="supplierCatalogWorkspace" aria-labelledby="supplier-catalog-title">
    ...
    <form ref={formRef} className="supplierCatalogCreate" onSubmit={create}><p><strong>{t("selectedCount")}: {selected.size}</strong></p>
      ...
      <div className="supplierCatalogItems" aria-live="polite">
        ...
        {selectedResults.map((item)=>{const preview=previews[item.supplierProductId];const isLowConfidence=Boolean(preview?.requiresReview)||Boolean(preview&&preview.classificationConfidence<0.62);return <article key={item.supplierProductId}><div><strong>{item.supplierProductId}</strong><span>{item.title}</span></div><dl><div><dt>{t("classification")}</dt><dd>{preview?.classificationStatus??t("reviewRequired")} {preview?.classificationConfidence!=null?`(${Math.round(preview.classificationConfidence*100)}%)`:""}</dd></div><div><dt>{t("categoryStatus")}</dt><dd>{preview?.suggestedCanonicalCategoryLabel??t("reviewRequired")}</dd></div><div><dt>{t("needsReview")}</dt><dd>{isLowConfidence?t("needsReview"):t("bulkStatusGood")}</dd></div><div><dt> {t("override")}</dt><dd><select name={`preview-category-${item.supplierProductId}`} defaultValue={preview?.suggestedCanonicalCategoryId??""}><option value="">{t("reviewRequired")}</option>{leaves.map((leaf)=><option key={leaf.id} value={leaf.id}>{leaf.label}</option>)}</select></dd></div><div>{isLowConfidence&&preview&&<strong>{t("quarantine")}</strong>}</div></dl></article>})}
      </div>
