...
export default function ProductPurchasePanel({ product, colors, sizes, options = [], variants = [], availabilityLabel }: { product: CartProduct; colors: string[]; sizes: string[]; options?: Option[]; variants?: Variant[]; availabilityLabel: string }) {
  ...
  const selectedPrice = selectedVariant?.priceOverride ?? product.price;
  const stock = isVariantProduct ? selectedVariant?.stock ?? 0 : product.stock;

  useEffect(() => {
    window.dispatchEvent(new CustomEvent("todijo:variant-price", { detail: { price: selectedPrice } }));
    setQuantity(1);
  }, [selectedPrice, selectedVariant?.id]);
