      ...
      <nav className="marketSecondaryNav" aria-label={header("categoryNavigation")}><div className="marketSecondaryInner">
        <div className="marketCategoryMenu" ref={categoryMenuRef} onMouseEnter={() => { cancelClose(); setCategoriesOpen(true); }} onMouseLeave={scheduleClose}>
          ...
          {categoriesOpen ? <section id="market-category-mega-menu" className="marketCategoryMegaMenu" aria-label={common("categories")} onKeyDown={(event) => { if (event.key === "Escape") setCategoriesOpen(false); }}>
            ...
            <div className="marketCategoryDetail">{(()=>{const category=DESKTOP_CATEGORY_TAXONOMY.find(item=>item.id===activeCategory)!;return <><header className="marketCategoryDetailHeader"><div><p>{category.label}</p><small>{header("discoverCategories")}</small></div><Link className="marketCategoryViewAll" href={categoryHref(category.label)} onClick={()=>setCategoriesOpen(false)}>{header("viewAll")}</Link></header><div className="marketCategoryContent" ref={categoryContentRef}>{category.groups.length?<div className="marketCategoryColumns">{category.groups.map(group=><section key={group.id}><h3 className="marketCategoryGroupHeading">{group.label}</h3>{group.items.map(item=><Link className="marketCategoryChildLink" key={item} href={categoryHref(item)} onClick={()=>setCategoriesOpen(false)}>{item}</Link>)}</section>)}</div>:<div className="marketCategoryEmpty"><p>{category.label}</p><Link href={categoryHref(category.label)} onClick={()=>setCategoriesOpen(false)}>{header("viewAll")} {category.label}</Link></div>}</div></>})()}</div>
          </section> : null}
        </div>
