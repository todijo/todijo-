...
test("GET/list are read-only and stale recovery stays mutation target-scoped",()=>{const jobs=read("lib/suppliers/supplier-catalog-jobs.ts"),list=jobs.slice(jobs.indexOf("export async function listCatalogImportJobs"),jobs.indexOf("type CatalogPricingAttempt")),detail=jobs.slice(jobs.indexOf("export async function readCatalogImportJob"),jobs.indexOf("export async function cancelCatalogImportJob"));assert.doesNotMatch(list,/recoverStaleCatalogClaims|\.update/);assert.doesNotMatch(detail,/recoverStaleCatalogClaims|\.update/);assert.match(jobs,/recoverStaleCatalogClaims\(db,\[jobId\]/);assert.match(jobs,/where:\{id:candidate\.id,status:"PENDING",job:\{status:\{in:\["PENDING","RUNNING"\]\}\}\}/);});
