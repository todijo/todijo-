...
test("desktop category rail opens its canonical menu and preserves localized routing", async ({ page }) => {
  ...
  const canonicalChildCategory = firstChildGroup.items[0];
  const canonicalChildId = subcategoryId(secondCategory.id, firstChildGroup.id, canonicalChildCategory);
  const localizedChildLabel = localizedCategoryLeafLabel(locale, secondCategory.id, firstChildGroup.id, canonicalChildCategory);
  const child = reopenedSecondMenu.getByRole("link", { name: localizedChildLabel, exact: true });
  await expect(child).toHaveAttribute("href", `/en/search?category=${encodeURIComponent(canonicalChildId)}`);
