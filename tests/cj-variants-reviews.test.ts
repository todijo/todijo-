...
test("real supplier import path persists the 4 color, 6 size, 23 variant matrix",async()=>{
  ...
  await importSupplierProduct(db,{id:"CJ",isConfigured:()=>true,getProduct:async()=>snapshot} as any,{copyRemote:async()=>{throw new Error("unexpected");}},{storeId:"store",connectionId:"platform-cj",ownerType:"PLATFORM",supplierProductId:"PID",sellingPrice:20,category:"women--outerwear--blazers"});
...
test("review failure does not break the real product import path",async()=>{
  ...
  const product=await importSupplierProduct(db,{id:"CJ",isConfigured:()=>true,getProduct:async()=>snapshot,getProductReviews:async()=>{throw new Error("CJ_REVIEW_TIMEOUT");}} as any,{copyRemote:async()=>{throw new Error("unexpected");}},{storeId:"store",connectionId:"platform-cj",ownerType:"PLATFORM",supplierProductId:"PID",sellingPrice:20,category:"women--outerwear--blazers"});
