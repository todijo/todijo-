...
test("mobile purchase bar keeps one visible primary action and safe content clearance", async () => {
  ...
  assert.match(purchase, /className="mobilePurchaseBar"><AddToCartButton compact/);
  assert.doesNotMatch(purchase, /mobilePurchaseThumb|mobilePurchaseSummary/);
  ...
  assert.match(css, /\.productDetailPage\{padding-bottom:calc\(132px \+ env\(safe-area-inset-bottom\)\)/);
...
test("mobile fullscreen maximizes the image without changing its aspect ratio", async () => {
  const css = await readFile("app/globals.css", "utf8");
  assert.match(css, /@media\(max-width:760px\)[\s\S]*?\.productLightbox\{height:100dvh;grid-template-rows:minmax\(0,1fr\) auto;[^}]*safe-area-inset-top/);
