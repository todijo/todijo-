...
test("homepage keeps filters and every category before the six-product hero mosaic", () => {
  ...
  assert.match(home, /selectDistinctHeroProducts\(heroProducts\.filter\(\(product\) => product\.image\)\)/);
...
test("hero layout has one dominant, one secondary, and four compact responsive slots", () => {
  const css = source("app/globals.css");
