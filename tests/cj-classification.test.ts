...
test("digital electronic watch prefers the specific digital-watch leaf over generic electronics watch",()=>{const result=classifyCjProduct(snapshot("Digital waterproof electronic watch","Waterproof digital electronic wrist watch","Electronics > Smart Electronics > Watches"));assert.equal(result.categoryId,"jewelry");assert.equal(result.subcategoryLabel,"Montres numériques");assert.equal(result.status,"SUGGESTED");assert.ok(result.confidence>=0.62);assert.equal(result.canonicalCategoryId!=null,true);assert.ok(result.evidence.some((entry)=>entry.includes("digital watch")));});

test("real CJ digital-watch titles remain auto-suggested even with generic electronics metadata",()=>{for(const title of ["Luminous Needle Waterproof Transparent Digital Electronic Watch","Fashion Men's Watch Unique LED Digital Watch Men Watch Electronic"]){const result=classifyCjProduct(snapshot(title,"Digital electronic wrist watch","Electronics > Smart Electronics > Watches"));assert.equal(result.categoryId,"jewelry",title);assert.equal(result.subcategoryLabel,"Montres numériques",title);assert.equal(result.status,"SUGGESTED",title);assert.ok(result.confidence>=0.62,title);}});
...
test("catalog preview respects CJ one-request-per-second sequencing",()=>{const route=readFileSync("app/api/admin/supplier-products/catalog-preview/route.ts","utf8");assert.doesNotMatch(route,/Promise\.all\(identifiers/);assert.match(route,/CJ_INTER_PRODUCT_DELAY_MS=1100/);assert.match(route,/for\(const \[index,identifier\] of identifiers\.entries\(\)\)/);assert.match(route,/await wait\(CJ_INTER_PRODUCT_DELAY_MS\)/);});
